...
func (c *ContainerManagerGRPCClient) GetContainerStatus(containerID *common.ContainerID) (*ContainerStatus, error) {
	...
	status := &ContainerStatus{
		...
		ExitStatus:  int(resp.ContainerStatus.ExitCode),
		Diagnostics: resp.ContainerStatus.Diagnostics,
	}

	if resp.ContainerStatus.StartTime != nil {
		status.StartTime = resp.ContainerStatus.StartTime.AsTime()
	}
	if resp.ContainerStatus.FinishTime != nil {
		status.FinishTime = resp.ContainerStatus.FinishTime.AsTime()
	}
