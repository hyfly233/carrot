...
type FIFOScheduler struct {
	rm scheduler.ResourceManagerInterface
}
...
func (s *FIFOScheduler) SetResourceManager(rm scheduler.ResourceManagerInterface) {
	s.rm = rm
}

// Schedule 为应用程序调度容器
func (s *FIFOScheduler) Schedule(app *scheduler.ApplicationInfo) ([]*common.Container, error) {
	if s.rm == nil {
...
func (s *FIFOScheduler) findAvailableNode(resource common.Resource) *scheduler.NodeInfo {
	if s.rm == nil {
		return nil
	}
