...
```json
{
  ...
  "configurations": [
    ...
    },
    {
      ...
      "request": "launch",
      "mode": "auto",
      "program": "${workspaceFolder}/cmd/rmnode/main.go",
      "args": [
        "-port",
        "8042",
        "-host",
...
```bash
dlv debug cmd/rmnode/main.go -- -port 8042 -host localhost -rm-url http://localhost:8088
```
...
./bin/rmnode -port 8042 > logs/nm-$(date +%Y%m%d-%H%M%S).log 2>&1 &
```
...
# 启动 NodeManager
echo "启动 NodeManager..."
./bin/rmnode -port 8042 -host localhost -rm-url http://localhost:8088 -memory 4096 -vcores 4 > $DEBUG_DIR/logs/nm.log 2>&1 &
NM_PID=$!
...
```bash
# 编译时保留调试信息
go build -gcflags="-N -l" -o bin/resourcemanager cmd/resourcemanager/main.go

# 使用 gdb 调试
gdb bin/resourcemanager
(gdb) run -port 8088
(gdb) bt  # 查看调用栈
```
...
这个调试指南应该能帮助您全面调试 Carrot YARN 项目的各个方面！
