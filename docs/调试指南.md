...
```bash
dlv debug cmd/node/main.go -- -port 8042 -host localhost -rm-url http://localhost:8088
```
...
./bin/node -port 8042 > logs/nm-$(date +%Y%m%d-%H%M%S).log 2>&1 &
```
...
# 启动 NodeManager
echo "启动 NodeManager..."
./bin/node -port 8042 -host localhost -rm-url http://localhost:8088 -memory 4096 -vcores 4 > $DEBUG_DIR/logs/nm.log 2>&1 &
NM_PID=$!
