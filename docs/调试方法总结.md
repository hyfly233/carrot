...
go test -bench=. -benchmem ./internal/rmnode
...
./bin/rmnode -port 8042 -host localhost -rm-url http://localhost:8088
```
...
```bash
# 查看进程状态
ps aux | grep -E 'resourcemanager|rmnode'
