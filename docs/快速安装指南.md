...
carrot-nm --generate-config > /etc/carrot/node.yaml
```
...
# 3. 启动 NodeManager
carrot-nm --config /etc/carrot/node.yaml &
...
services:
  ...
  nodemanager1:
    ...
    environment:
      ...
      - NM_VCORES=2
    volumes:
      - ./config/nm.yaml:/etc/carrot/node.yaml
...
tail -f /var/log/carrot/node.log
```
...
# 检查 NodeManager 配置
grep resourcemanager /etc/carrot/node.yaml
