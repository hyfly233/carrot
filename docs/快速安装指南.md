...
docker pull carrot/rmnode:latest
...
carrot-nm --generate-config > /etc/carrot/rmnode.yaml
```
...
# 3. 启动 NodeManager
carrot-nm --config /etc/carrot/rmnode.yaml &
...
services:
  ...
  nodemanager1:
    ...
    environment:
      ...
      - NM_VCORES=2
    volumes:
      - ./config/nm.yaml:/etc/carrot/rmnode.yaml
...
tail -f /var/log/carrot/rmnode.log
```
...
# 检查 NodeManager 配置
grep resourcemanager /etc/carrot/rmnode.yaml
