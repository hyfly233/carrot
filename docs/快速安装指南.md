...
docker pull carrot/rmnm:latest
...
carrot-nm --generate-config > /etc/carrot/rmnm.yaml
```
...
# 3. 启动 NodeManager
carrot-nm --config /etc/carrot/rmnm.yaml &
...
services:
  ...
  nodemanager1:
    ...
    environment:
      ...
      - NM_VCORES=2
    volumes:
      - ./config/nm.yaml:/etc/carrot/rmnm.yaml
...
tail -f /var/log/carrot/rmnm.log
```
...
# 检查 NodeManager 配置
grep resourcemanager /etc/carrot/rmnm.yaml
